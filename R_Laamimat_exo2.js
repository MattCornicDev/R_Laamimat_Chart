...
for(let i=1 ; i < data.length;i++)
{
    ...
    donnees.addRow([(data[i][0]),parseInt(data[i][1]),parseInt(data[i][2]),parseInt(data[i][3]),parseInt(data[i][4]),(parseInt(data[i][5])),(parseInt(data[i][6])),parseInt(data[i][7]),parseInt(data[i][8]),(parseInt(data[i][9])),(parseInt(data[i][10])),(parseInt(data[i][11])),Math.floor(somme/11) ]);
...
function traceDiagramme(data)
{
    ...
    var tableauDeBord = new google.visualization.Dashboard(document.getElementById('dashboard'));
    ...
    var progDiag = new google.visualization.ChartWrapper({
        ...
        options:{
            ...
            title: 'Evolution de la population européenne entre 2008 et 2018', 
                vAxis: { title: "Millions d\'habitants" },
                hAxis: { title: "pays" },
                seriesType: "bars" 
